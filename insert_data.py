...
for pokemon in poke_data:
    pokenom_id = pokemon.get("id")
    name = pokemon.get("name")
    type = pokemon.get("type")
    height = pokemon.get("height")
    weight = pokemon.get("weight")
    pokemon_values.append(f'({pokenom_id}, "{name}",{height},{weight})')
    types_values.append(f'("{type}")')
    pokemon_type_values.append(f'("{type}", {pokenom_id})')
    ...
    for trainer in owned_by:
        ...
        pokemon_trainer_values.append(f'({pokenom_id}, "{trainer_name}")')
